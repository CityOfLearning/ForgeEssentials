...
public class ConfigServerVote extends ConfigLoaderBase {
	...
	public void load(Configuration config, boolean isReload) {
		...
		hostname = config.get(subcat, "hostname", "", "Set this to the hostname (or IP address) of your server.")
				.getString();
		port = config.get(subcat, "port", "8192", "The port which the vote receiver should listen on.").getInt();

		allowOfflineVotes = config
				.get(category, "allowOfflineVotes", true, "If false, votes of offline players will be canceled.")
				.getBoolean(true);
		msgAll = config.get(category, "msgAll", "%player has voted for this server on %service.",
				"You can use color codes (&), %player and %service").getString();
		msgVoter = config.get(category, "msgVoter", "Thanks for voting for our server!",
				"You can use color codes (&), %player and %service").getString();
