...
public class CommandDrop extends ForgeEssentialsCommandBase {
		...
		if ((par5 != 0) || (par6 != 0)) {
			if (d1 < par5) {
				throw new NumberInvalidException("commands.generic.double.tooSmall",
						new Object[] { Double.valueOf(d1), Integer.valueOf(par5) });
			}

			if (d1 > par6) {
				throw new NumberInvalidException("commands.generic.double.tooBig",
						new Object[] { Double.valueOf(d1), Integer.valueOf(par6) });
			}
		}
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		...
		World world = null;
		int x = (int) func_82368_a(sender, 0.0D, args[0]);
		int y = (int) func_82367_a(sender, 0.0D, args[1], 0, 0);
		int z = (int) func_82368_a(sender, 0.0D, args[2]);

		if (sender instanceof DedicatedServer) {
			world = ((DedicatedServer) sender).worldServerForDimension(0);
		} else if (sender instanceof EntityPlayerMP) {
			world = ((Entity) sender).worldObj;
			x = (int) func_82368_a(sender, ((Entity) sender).posX, args[0]);
			y = (int) func_82367_a(sender, ((Entity) sender).posY, args[1], 0, 0);
			z = (int) func_82368_a(sender, ((Entity) sender).posZ, args[2]);
		} else if (sender instanceof TileEntity) {
			world = ((TileEntity) sender).getWorld();
			x = (int) func_82368_a(sender, ((TileEntity) sender).getPos().getX(), args[0]);
			y = (int) func_82367_a(sender, ((TileEntity) sender).getPos().getY(), args[1], 0, 0);
			z = (int) func_82368_a(sender, ((TileEntity) sender).getPos().getZ(), args[2]);
		}
		...
		int var9 = parseInt(args[5], 1,
				GameData.getItemRegistry().getObject(new ResourceLocation(var7)).getItemStackLimit());
		ItemStack tmpStack;
		...
		if (tileEntity instanceof TileEntityChest) {
			...
			for (int slot = 0; slot < var10.getSizeInventory(); ++slot) {
				...
				if ((var10.getStackInSlot(slot).getUnlocalizedName() == var7)
						&& (var10.getStackInSlot(slot).getItemDamage() == var8)) {
			...
			for (int slot = 0; slot < var13.getSizeInventory(); ++slot) {
				...
				if ((var13.getStackInSlot(slot).getUnlocalizedName() == var7)
						&& (var13.getStackInSlot(slot).getItemDamage() == var8)) {
			...
			for (int slot = 0; slot < var14.getSizeInventory(); ++slot) {
				...
				if ((var14.getStackInSlot(slot).getUnlocalizedName() == var7)
						&& (var14.getStackInSlot(slot).getItemDamage() == var8)) {
			...
			for (int slot = 0; slot < var12.getSizeInventory(); ++slot) {
				...
				if ((var12.getStackInSlot(slot).getUnlocalizedName() == var7)
						&& (var12.getStackInSlot(slot).getItemDamage() == var8)) {
