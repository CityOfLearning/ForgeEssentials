...
public class CommandKit extends ParserCommandBase implements ConfigurableCommand {
	...
	public void loadConfig(Configuration config, String category) {
		kitForNewPlayers = config.get(category, "kitForNewPlayers", "",
				"Name of kit to issue to new players. If this is left blank, it will be ignored.").getString();
	}
		...
		case "set":
			QuestionerCallback callback = response -> {
				...
				if (cooldown < 0) {
					...
					arguments.confirm("Kit %s saved with cooldown %s", kitName,
