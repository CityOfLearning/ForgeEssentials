	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		...
		if ((sender instanceof EntityPlayerMP) && args[0].equalsIgnoreCase("[CONSOLE]")) {
			...
			MinecraftServer.getServer().getCommandManager()
					.executeCommand(new DoAsCommandSender(APIRegistry.IDENT_SERVER, player), cmd);
		}
		...
		if (player != null) {
			ChatOutputHandler.chatWarning(player,
					Translator.format("Player %s is attempting to issue a command as you.", sender.getName()));
