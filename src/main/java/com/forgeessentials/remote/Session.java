...
public class Session implements Runnable, RemoteSession {
	...
	protected void processMessage(String message) throws IOException {
		try {
			...
			if ((ident != null) && MinecraftServer.getServer().getConfigurationManager().getBannedPlayers()
		...
		try {
			...
			while (true) {
				try {
					...
					if (msg == null) {
						// LoggingHandler.felog.warn("[remote] Connection
						// closed: " + getRemoteHostname());
						break;
					}
					processMessage(msg);
				} catch (IOException e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.forgeessentials.api.remote.RemoteSession#sendMessage(java.lang.
	 * Object)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.forgeessentials.api.remote.RemoteSession#transformRemoteRequest(com.
