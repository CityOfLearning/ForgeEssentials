...
public class RemoteCommandSender extends DoAsCommandSender {
	...
	private RemoteCommandSender(RemoteSession session) {
		...
		if (session.getUserIdent() != null) {
			sender = session.getUserIdent().getFakePlayer();
		} else {
			sender = FakePlayerFactory.get(ServerUtil.getOverworld(), ModuleRemote.FAKEPLAYER);
		}
	...
	public void addChatMessage(IChatComponent chatComponent) {
		// TODO: Instead of directly sending the messages to the client, cache
		// them and send them all after the running
		...
		if (!session.isClosed()) {
			try {
				...
				ChatResponse msg = new ChatResponse(null,
						ChatOutputHandler.stripFormatting(chatComponent.getUnformattedText()));
