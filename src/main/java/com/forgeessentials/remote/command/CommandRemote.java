...
public class CommandRemote extends ParserCommandBase {
	...
	public void parse(CommandParserArgs args) throws CommandException {
		if (args.isTabCompletion && (args.size() == 1)) {
			args.tabCompletion = ForgeEssentialsCommandBase.getListOfStringsMatchingLastWord(args.peek(),
					parseMainArgs);
			return;
		}
		if (args.isEmpty()) {
			...
			case "block": {
				...
				args.confirm("User %s has been blocked from remote until he generates a new passkey",
			...
			case "qr": {
				...
				try {
					if (!PlayerInfo.get(ident.getPlayerMP()).getHasFEClient()) {
						...
						String url = ("https://chart.googleapis.com/chart?cht=qr&chld=M|4&chs=547x547&chl="
								+ connectString).replaceAll("\\|", "%7C");
	...
	public void showPasskey(CommandParserArgs args, UserIdent ident, boolean hideKey) {
		...
		ChatOutputHandler.sendMessage(args.sender,
				new ChatComponentText("Port = " + ModuleRemote.getInstance().getPort()));
	}
