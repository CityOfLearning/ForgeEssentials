...
public class Mailer extends ServerEventHandler {
	...
	public static void sendMail(UserIdent sender, UserIdent recipent, String message) {
		...
		if (recipent.hasPlayer()) {
			ChatOutputHandler.chatNotification(recipent.getPlayer(), Translator.format("You have a new mail from %s",
					sender == null ? "the server" : sender.getUsernameOrUuid()));
		}
	}
	...
	public void playerLoggedInEvent(PlayerLoggedInEvent event) {
		...
		String message = Translator.format("You hav unread mails from %s. Use /mail to read.",
