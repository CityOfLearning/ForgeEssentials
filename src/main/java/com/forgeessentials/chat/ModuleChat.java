...
public class ModuleChat {
	...
	public static IChatComponent appendGroupPrefixSuffix(IChatComponent header, UserIdent ident, boolean isSuffix) {
		...
		for (GroupEntry group : APIRegistry.perms.getServerZone().getAdditionalPlayerGroups(ident,
			...
			if (text != null) {
				IChatComponent component = clickChatComponent(text, Action.SUGGEST_COMMAND,
						"/gmsg " + group.getGroup() + " ");
				if (header == null) {
					header = component;
				} else {
	...
	public static IChatComponent getChatHeader(UserIdent ident) {
		...
		IChatComponent playerPrefix = clickChatComponent(getPlayerPrefixSuffix(ident, false), Action.SUGGEST_COMMAND,
				playerCmd);
		IChatComponent playerText = clickChatComponent(playerFormat + playerName, Action.SUGGEST_COMMAND, playerCmd);
		IChatComponent playerSuffix = clickChatComponent(getPlayerPrefixSuffix(ident, true), Action.SUGGEST_COMMAND,
				playerCmd);
	...
	public static void tell(ICommandSender sender, IChatComponent message, ICommandSender target) {
		ChatComponentTranslation sentMsg = new ChatComponentTranslation("commands.message.display.incoming",
				new Object[] { sender.getDisplayName(), message.createCopy() });
	...
	public void chatEvent(ServerChatEvent event) {
		...
		if (textFormats != null) {
			ChatOutputHandler.applyFormatting(messageComponent.getChatStyle(),
					ChatOutputHandler.enumChatFormattings(textFormats));
		}
	...
	public void serverStarting(FEModuleServerInitEvent e) {
		...
		APIRegistry.perms.registerPermission(PERM_URL, PermissionLevel.TRUE,
