...
public class PlayerLogger extends ServerEventHandler implements Runnable {
	...
	protected CompoundPredicate getActionPredicate(Root<? extends Action> root, WorldArea area, Date startTime,
			Date endTime) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CompoundPredicate predicate = (CompoundPredicate) cb.and();
		if (area != null) {
			predicate.getExpressions()
					.add(cb.equal(root.<Integer> get(Action_.world.getName()), cb.literal(area.getDimension())));
	...
	protected CompoundPredicate getActionPredicate(Root<? extends Action> root, WorldPoint point, Date startTime,
			Date endTime) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CompoundPredicate predicate = (CompoundPredicate) cb.and();
		if (point != null) {
			predicate.getExpressions()
					.add(cb.equal(root.<Integer> get(Action_.world.getName()), cb.literal(point.getDimension())));
	...
	protected void loadDatabase() {
		...
		entityManagerFactory = Persistence.createEntityManagerFactory("playerlogger_" + PlayerLoggerConfig.databaseType,
				properties);
		// entityManagerFactory =
		// Persistence.createEntityManagerFactory("playerlogger_eclipselink_" +
	...
	public void playerInteractEvent(PlayerInteractEvent event) {
		if ((FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT)
				|| ((event.useBlock == Result.DENY) && (event.useItem == Result.DENY))) {
			return;
		}
	...
	public void playerPostInteractEvent(PlayerPostInteractEvent event) {
		if (event.stack != null) {
			...
			if ((item instanceof ItemBlock) || (item instanceof ItemRedstone) || (item instanceof ItemBed)
					|| (item instanceof ItemDoor) || (item instanceof ItemSkull)) {
				return;
			}
		}
		...
		while (!eventQueue.isEmpty()) {
			synchronized (this) {
				...
				if (!em.isOpen()) {
					...
					try {
						...
						LoggingHandler.felog
								.error("[PL] ------------------------------------------------------------------------");
						LoggingHandler.felog
								.error("[PL] Fatal error! Database connection was lost and could not be reestablished");
				...
				try {
					...
					em.getTransaction().commit();
					// System.out.println(String.format("%d: Wrote %d
					// playerlogger entries", System.currentTimeMillis()
					// % (1000 * 60), count));
				} catch (Exception e1) {
