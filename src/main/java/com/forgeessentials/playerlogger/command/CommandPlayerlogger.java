		...
		case "purge":
			if (arguments.isEmpty()) {
				...
				QuestionerCallback handler = response -> {
					...
					TaskRegistry.runLater(() -> ModulePlayerLogger.getLogger().purgeOldData(startTime));
				};
				if (arguments.sender instanceof MinecraftServer) {
					handler.respond(true);
				} else {
					Questioner.addChecked(arguments.sender,
							Translator.format("Really purge all playerlogger data before %s?", startTimeStr), handler);
				}
			}
			break;
		default:
