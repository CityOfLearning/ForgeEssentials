...
public class CommandFaction extends ParserCommandBase {
	...
	public static void parseCreate(CommandParserArgs arguments) throws CommandException {
		...
		if (arguments.senderPlayer != null) {
			...
			APIRegistry.perms.getServerZone().addPlayerToGroup(arguments.ident,
	...
	public static void parseDelete(final CommandParserArgs arguments, final String faction) throws CommandException {
		...
		QuestionerCallback callback = response -> {
			if (response == null) {
				return;
			} else if (!response) {
			...
			for (Entry<UserIdent, Set<String>> player1 : APIRegistry.perms.getServerZone().getPlayerGroups()
					.entrySet()) {
				for (Iterator<String> it1 = player1.getValue().iterator(); it1.hasNext();) {
					if (factionGroup.equals(it1.next())) {
						it1.remove();
					}
				}
			}
			for (Entry<UserIdent, Set<String>> player2 : APIRegistry.perms.getServerZone().getPlayerGroups()
					.entrySet()) {
				if (player2.getValue().remove(factionGroup) && player2.getKey().hasPlayer()) {
					ChatOutputHandler.chatNotification(player2.getKey().getPlayer(),
							Translator.format("Faction %s has been deleted", faction));
				}
				for (Iterator<String> it2 = player2.getValue().iterator(); it2.hasNext();) {
					if (it2.next().startsWith(ModuleFactions.RANK_PREFIX)) {
						it2.remove();
					}
				}
			}
	...
	public static void parseJoin(final CommandParserArgs arguments) throws CommandException {
		...
		String message = Translator.format("Allow %s to join the faction %s?", arguments.ident.getUsernameOrUuid(),
				faction);
		QuestionerCallback callback = response -> {
			if (response == null) {
	...
	public static void parseLeave(CommandParserArgs arguments, String faction) throws CommandException {
		...
		APIRegistry.perms.getServerZone().removePlayerFromGroup(arguments.ident,
				ModuleFactions.getFactionGroup(faction));
