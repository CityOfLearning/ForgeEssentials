...
public class ConfigTickets extends ConfigLoaderBase {
	...
	public void load(Configuration config, boolean isReload) {
		...
		ModuleTickets.categories = Arrays.asList(
				config.get(cat, "categories", new String[] { "griefing", "overflow", "dispute" }).getStringList());
	...
	public void save(Configuration config) {
		...
		config.get(cat, "categories", new String[] { "griefing", "overflow", "dispute" })
				.set(ModuleTickets.categories.toArray(new String[0]));
