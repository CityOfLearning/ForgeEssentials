...
public class HelpFixer extends CommandHelp {
		...
		if (hideWorldEditCommands) {
			for (Iterator<ICommand> it = list.iterator(); it.hasNext();) {
				...
				if (command.getClass().getName().startsWith("com.sk89q.worldedit")
						&& !command.getCommandName().equals("/help")) {
					it.remove();
				}
			}
		}
		// Ok there is some retard who thinks he should implement ICommand
		// instead of extending CommandBase and then
		// fails to properly implement compareTo (it always returns 0).
		// So to prevent crashes from these kind of things, we just provide our
		// own comparator
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		...
		ChatComponentTranslation msg = new ChatComponentTranslation("commands.help.header",
				new Object[] { Integer.valueOf(startPage + 1), Integer.valueOf(i + 1) });
		...
		for (int index = startPage * cmdsPerPage; index < endIndex; ++index) {
			...
			msg2.getChatStyle().setChatClickEvent(
