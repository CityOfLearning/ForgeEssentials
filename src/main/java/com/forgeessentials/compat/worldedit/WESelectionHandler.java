...
public class WESelectionHandler implements ISelectionProvider {
	...
	public Selection getSelection(EntityPlayerMP player) {
		...
		if (regionSelector instanceof CuboidRegionSelector) {
			...
			try {
				...
				return new Selection(world, new Point(wepos1.getBlockX(), wepos1.getBlockY(), wepos1.getBlockZ()),
			...
			try {
				...
				// new Point(c.getBlockX() - r.getBlockX(), c.getBlockY() -
				// r.getBlockY(), c.getBlockZ() -
				// r.getBlockZ()),
				// new Point(c.getBlockX() + r.getBlockX(), c.getBlockY() +
				// r.getBlockY(), c.getBlockZ() +
				// r.getBlockZ()));
			...
			try {
				...
				// return new Selection(world,
				// new Point(c.getBlockX() - r.getBlockX(), cyl.getMinimumY(),
				// c.getBlockZ() - r.getBlockZ()),
				// new Point(c.getBlockX() + r.getBlockX(), cyl.getMaximumY(),
				// c.getBlockZ() + r.getBlockZ()));
			} catch (IncompleteRegionException e) {
				return null;
			}
	...
	public void select(EntityPlayerMP player, int dimension, AreaBase area) {
		...
		selector.selectPrimary(
				new Vector(area.getLowPoint().getX(), area.getLowPoint().getY(), area.getLowPoint().getZ()), null);
		selector.selectSecondary(
				new Vector(area.getHighPoint().getX(), area.getHighPoint().getY(), area.getHighPoint().getZ()), null);
	...
	public void setEnd(EntityPlayerMP player, Point end) {
		...
		session.getRegionSelector(session.getSelectionWorld())
				.selectSecondary(new Vector(end.getX(), end.getY(), end.getZ()), null);
	}
	...
	public void setStart(EntityPlayerMP player, Point start) {
		LocalSession session = ForgeWorldEdit.inst.getSession(player);
		session.getRegionSelector(session.getSelectionWorld())
				.selectPrimary(new Vector(start.getX(), start.getY(), start.getZ()), null);
	}
