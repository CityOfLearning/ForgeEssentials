...
public class FELaunchHandler implements ITweaker {
	...
	private void loadLibraries(LaunchClassLoader classLoader) {
		...
		for (File f : files) {
			try {
				...
				throw new RuntimeException(String.format("[ForgeEssentials] Error adding library %s to classpath: %s",
	...
	private void loadModules(LaunchClassLoader classLoader) {
		for (File f : moduleDirectory.listFiles(JAR_FILTER)) {
			try {
				...
				throw new RuntimeException(String.format("[ForgeEssentials] Error adding module %s to classpath: %s",
						f.getAbsolutePath(), e.getMessage()));
			}
		}
	}
	...
	public boolean shouldExtractLibraries() {
		// boolean runtimeDeobfEnabled = (!(boolean)
