...
public abstract class MixinItemInWorldManager {
	...
	public boolean activateBlockOrUseItem(EntityPlayer player, World worldIn, ItemStack stack, BlockPos pos,
		...
		if (gameType == WorldSettings.GameType.SPECTATOR) {
			...
			if ((stack != null)
					&& stack.getItem().onItemUseFirst(stack, player, worldIn, pos, side, offsetX, offsetY, offsetZ)) {
				MinecraftForge.EVENT_BUS.post(new PlayerPostInteractEvent(player, worldIn, worldIn.getBlockState(pos),
						pos, side, offsetX, offsetY, offsetZ));
			...
			if (useBlock) {
				if (event.useBlock != net.minecraftforge.fml.common.eventhandler.Event.Result.DENY) {
					result = iblockstate.getBlock().onBlockActivated(worldIn, pos, iblockstate, player, side, offsetX,
							offsetY, offsetZ);
					// PATCH: Add event to get actual result of interaction
					if (result) {
						MinecraftForge.EVENT_BUS.post(new PlayerPostInteractEvent(player, worldIn, iblockstate, pos,
								side, offsetX, offsetY, offsetZ));
					}
				} else {
					if (thisPlayerMP.playerNetServerHandler != null) {
						thisPlayerMP.playerNetServerHandler.sendPacket(new S23PacketBlockChange(theWorld, pos));
