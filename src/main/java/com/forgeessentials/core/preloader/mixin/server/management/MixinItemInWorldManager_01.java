...
public abstract class MixinItemInWorldManager_01 {
	...
	@Overwrite
	public boolean activateBlockOrUseItem(EntityPlayer player, World world, ItemStack item, BlockPos pos,
			EnumFacing side, float dx, float dy, float dz) {
		if (gameType == WorldSettings.GameType.SPECTATOR) {
			...
			net.minecraftforge.event.entity.player.PlayerInteractEvent event = net.minecraftforge.event.ForgeEventFactory
					.onPlayerInteract(player,
							net.minecraftforge.event.entity.player.PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK, world,
							pos, side);
			if (event.isCanceled()) {
			...
			// PATCH: Fix a Forge bug allowing onItemUseFirst to trigger even if
			// event.useItem is set to DENY
			if ((item != null) && (event.useItem != Event.Result.DENY)
					&& item.getItem().onItemUseFirst(item, player, world, pos, side, dx, dy, dz)) {
				...
				if (event.useBlock != net.minecraftforge.fml.common.eventhandler.Event.Result.DENY) {
					...
					if (result) {
						MinecraftForge.EVENT_BUS
								.post(new PlayerPostInteractEvent(player, world, blackState, pos, side, dx, dy, dz));
					}
				} else {
					thisPlayerMP.playerNetServerHandler.sendPacket(new S23PacketBlockChange(theWorld, pos));
			...
			if ((item != null) && !result
					&& (event.useItem != net.minecraftforge.fml.common.eventhandler.Event.Result.DENY)) {
				...
				if (result) {
					MinecraftForge.EVENT_BUS
							.post(new PlayerPostInteractEvent(player, world, item, pos, side, dx, dy, dz));
				}
			}
			return result;
		}
