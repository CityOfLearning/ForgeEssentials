...
public final class MapperUtil {
	...
	public static boolean chunkExists(WorldServer world, int cx, int cz) {
		return RegionFileCache.createOrLoadRegionFile(world.getChunkSaveLocation(), cx, cz).chunkExists(cx & 0x1F,
				cz & 0x1F);
	}
	...
	public static Color getBlockColor(Block block, int meta) {
		...
		// chunk.getBlockMetadata(ix, iy, iz));
	...
	public static Chunk loadChunk(WorldServer world, int cx, int cz) {
		Chunk chunk = world.theChunkProviderServer.id2ChunkMap.getValueByKey(ChunkCoordIntPair.chunkXZ2Int(cx, cz));
		if (chunk != null) {
			return chunk;
		}
		try {
	...
	public static void loadColorScheme(InputStream stream) {
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				c[1] = new Color(Integer.parseInt(split[9]), Integer.parseInt(split[10]), Integer.parseInt(split[11]),
						Integer.parseInt(split[12]));
				c[2] = new Color(Integer.parseInt(split[13]), Integer.parseInt(split[14]), Integer.parseInt(split[15]),
	...
	public static BufferedImage renderRegion(WorldServer world, int regionX, int regionZ) {
		...
		// ".mca");
		BufferedImage image = new BufferedImage(MapperUtil.REGION_BLOCKS, MapperUtil.REGION_BLOCKS,
