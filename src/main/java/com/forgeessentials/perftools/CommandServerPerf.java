...
public class CommandServerPerf extends ForgeEssentialsCommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		...
		ChatOutputHandler.chatNotification(sender,
				"Total: " + (Runtime.getRuntime().totalMemory() / 1024 / 1024) + " MiB");
		ChatOutputHandler.chatNotification(sender,
				"Free: " + (Runtime.getRuntime().freeMemory() / 1024 / 1024) + " MiB");
		long used = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();
		ChatOutputHandler.chatNotification(sender, "Used: " + (used / 1024 / 1024) + " MiB");
		ChatOutputHandler.chatNotification(sender, "Average tick time: "
				+ formatNumbers.format(func_120035_a(MinecraftServer.getServer().tickTimeArray) * 1.0E-6D) + " ms");
