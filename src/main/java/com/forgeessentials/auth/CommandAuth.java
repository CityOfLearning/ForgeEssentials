...
public class CommandAuth extends ForgeEssentialsCommandBase {
	private static String[] playerCommands = new String[] { "help", "login", "register", "changepass", "kick",
			"setpass", "unregister" };
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
		...
		case 2:
			if (args[0].equalsIgnoreCase("kick") || args[0].equalsIgnoreCase("setpass")
				...
				list.addAll(getListOfStringsMatchingLastWord(args,
		...
		if (args.length == 2) {
			...
			if (args[0].equalsIgnoreCase("kick")) {
				if (!isLogged) {
					...
					ChatOutputHandler.chatConfirmation(sender, Translator
							.format("Player %s was logged out from the authentication service.", player.getName()));
	...
	public void processCommandPlayer(EntityPlayerMP sender, String[] args) throws CommandException {
		...
		if (args.length == 2) {
			...
			// parse ./auth unregister
				...
				PasswordManager.setPassword(player.getPersistentID(), null);
				ChatOutputHandler.chatConfirmation(sender, Translator
						.format("Player %s has been removed from the authentication service.", player.getName()));
				return;
			}
