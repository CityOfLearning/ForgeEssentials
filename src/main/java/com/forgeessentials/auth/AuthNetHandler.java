...
public class AuthNetHandler implements IMessageHandler<Packet6AuthLogin, IMessage> {
	...
	public IMessage onMessage(Packet6AuthLogin message, MessageContext ctx) {
		...
		case 1:
			if (message.hash != "") {
				if (PasswordManager.hasSession(UserIdent.get(ctx.getServerHandler().playerEntity).getUuid(),
						UUID.fromString(message.hash))) {
					ModuleAuth.authenticate(UserIdent.get(ctx.getServerHandler().playerEntity).getUuid());
					APIRegistry.getFEEventBus().post(
							new PlayerAuthLoginEvent.Success(ctx.getServerHandler().playerEntity, Source.AUTOLOGIN));
				}
			}
			break;
		default:
