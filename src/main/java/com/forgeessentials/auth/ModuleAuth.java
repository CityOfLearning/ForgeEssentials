...
public class ModuleAuth extends ConfigLoaderBase {
	...
	public void load(Configuration config, boolean isReload) {
		...
		EncryptionHelper.algorithm = config.get(CONFIG_CATEGORY, "encryptionAlgorithm", "SHA1", CFG_DESC_encryption)
				.getString();
		canMoveWithoutLogin = config.get(CONFIG_CATEGORY, "canMoveWithoutLogin", false, CFG_DESC_canMoveWithoutLogin)
				.getBoolean(false);
		allowOfflineRegistration = config.get(CONFIG_CATEGORY, "allowOfflineReg", false, CFG_DESC_allowOfflineReg)
				.getBoolean(false);
		forceEnabled = config.get(CONFIG_CATEGORY, "forceEnable", false, CFG_DESC_forceEnable).getBoolean(false);
		PasswordManager.setSalt(
				config.get(CONFIG_CATEGORY, "salt", EncryptionHelper.generateSalt(), CFG_DESC_salt).getString());
		...
		AuthEventHandler.vipSlots = config
				.get(CONFIG_CATEGORY_LISTS, "vipslots", 0, "Amount of slots reserved for VIP players.").getInt();

		config.addCustomCategoryComment(CONFIG_CATEGORY_LISTS + ".kickmsg", CFG_DESC_kickMsg);
		AuthEventHandler.playerBannedMessage = config
				.get(CONFIG_CATEGORY_LISTS + ".kick", "bannedmsg", "You have been banned from this server.")
				.getString();
		AuthEventHandler.nonVipKickMessage = config
				.get(CONFIG_CATEGORY_LISTS + ".kick", "notVIPmsg", "This server is full, and you are not a VIP.")
				.getString();
		allowAutoLogin = config.get(CONFIG_CATEGORY, "allowAutoLogin", true, CFG_DESC_autologin).getBoolean();

		checkVanillaAuthStatus = config.get(CONFIG_CATEGORY, "autoEnable", false, CFG_DESC_autoEnable)
				.getBoolean(false);
