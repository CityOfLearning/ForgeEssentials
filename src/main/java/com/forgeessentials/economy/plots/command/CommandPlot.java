...
public class CommandPlot extends ParserCommandBase {
		...
		if (plot.hasOwner()) {
			...
			if (plot.getOwner().hasPlayer()) {
				ChatOutputHandler.chatConfirmation(plot.getOwner().getPlayerMP(),
						Translator.format("You sold plot \"%s\" to %s for %s", //
	...
	public static void parseBuyStart(final CommandParserArgs arguments) throws CommandException {
		...
		if (!arguments.isEmpty()) {
			...
			if ((sellPrice >= 0) && (sellPrice < buyPrice)) {
				arguments.notify(Translator.format("%s is above the plots default price of %s",
		...
		if (!plot.hasOwner()) {
			...
			if (buyPrice != sellPrice) {
				arguments.error(Translator.format("The fixed price of this plot is %s.",
		...
		QuestionerCallback handler = response -> {
			...
			if ((sellPrice < 0) || (sellPrice > buyPrice)) {
				...
				QuestionerCallback handler1 = response1 -> {
					if (response1 == null) {
						arguments.error(Translator.format("%s did not respond to your buy request",
								plot.getOwner().getUsernameOrUuid()));
						return;
				...
				Questioner.addChecked(plot.getOwner().getPlayerMP(), message, handler1, 60);
			} else {
	...
	public static void parsePerms(CommandParserArgs arguments, boolean userPerms) throws CommandException {
		...
		case "interact":
			...
			break;
		case "chest":
			plot.setPermission(ModuleProtection.getBlockBreakPermission(Blocks.chest, 0) + Zone.ALL_PERMS, userPerms,
					allow);
			plot.setPermission(ModuleProtection.getBlockBreakPermission(Blocks.chest, 0) + Zone.ALL_PERMS, userPerms,
					allow);
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.chest, 0) + Zone.ALL_PERMS, userPerms,
					allow);
			plot.setPermission(ModuleProtection.getBlockBreakPermission(Blocks.trapped_chest, 0) + Zone.ALL_PERMS,
					userPerms, allow);
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.trapped_chest, 0) + Zone.ALL_PERMS,
					userPerms, allow);
			arguments.confirm(msgBase + "to interact with chests");
			break;
		case "button":
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.wooden_button, 0) + Zone.ALL_PERMS,
					userPerms, allow);
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.stone_button, 0) + Zone.ALL_PERMS,
					userPerms, allow);
			arguments.confirm(msgBase + "to interact with buttons");
			break;
		case "lever":
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.lever, 0) + Zone.ALL_PERMS, userPerms,
					allow);
			arguments.confirm(msgBase + "to interact with levers");
			break;
		case "door":
			plot.setPermission(ModuleProtection.getBlockInteractPermission(Blocks.oak_door, 0) + Zone.ALL_PERMS,
					userPerms, allow);
	...
	public static void parseSetFee(CommandParserArgs arguments) throws CommandException {
		...
		arguments.confirm(Translator.format("Set plot price to %s and timeout to %d",
