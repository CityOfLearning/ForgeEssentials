...
public class ModuleEconomy extends ServerEventHandler implements Economy, ConfigLoader {
	...
	public static int countInventoryItems(EntityPlayer player, ItemStack itemType) {
		...
		for (int slot = 0; slot < player.inventory.mainInventory.length; slot++) {
			...
			if ((stack != null) && (stack.getItem() == itemType.getItem())
	...
	public static Long getItemPrice(ItemStack itemStack, UserIdent ident) {
		return ServerUtil
				.tryParseLong(APIRegistry.perms.getUserPermissionProperty(ident, getItemPricePermission(itemStack)));
	}
	...
	public void load(Configuration config, boolean isReload) {
			...
			for (Entry<String, Property> entry : category.entrySet()) {
				for (Item item : GameData.getItemRegistry().typeSafeIterable()) {
					if (entry.getKey().equals(item.getUnlocalizedName())) {
						...
						config.get(CATEGORY_ITEM, id, DEFAULT_ITEM_PRICE)
								.set(entry.getValue().getInt(DEFAULT_ITEM_PRICE));
						break;
					}
				}
			}
		...
		for (Entry<String, Property> entry : category.entrySet()) {
			APIRegistry.perms.registerPermissionProperty(PERM_PRICE + "." + entry.getKey(),
					Integer.toString(entry.getValue().getInt(DEFAULT_ITEM_PRICE)));
		}
	}
	...
	public void onDeath(LivingDeathEvent e) {
		if (e.entity instanceof EntityPlayerMP) {
			...
			ChatOutputHandler.chatNotification(e.entity,
		...
		if (e.source.getEntity() instanceof EntityPlayerMP) {
			...
			if (bounty > 0) {
				...
				if (APIRegistry.perms.checkUserPermission(killer, PERM_BOUNTY_MESSAGE)) {
					ChatOutputHandler.chatNotification(killer.getPlayer(), Translator
	...
	public void serverStarting(FEModuleServerInitEvent event) {
		...
		APIRegistry.perms.registerPermission(PERM_BOUNTY_MESSAGE, PermissionLevel.TRUE,
