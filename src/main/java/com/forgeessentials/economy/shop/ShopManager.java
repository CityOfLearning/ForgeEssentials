...
public class ShopManager extends ServerEventHandler implements ConfigLoader {
	...
	public void attackEntityEvent(final AttackEntityEvent event) {
		...
		if (!APIRegistry.perms.checkUserPermission(UserIdent.get(event.entityPlayer), new WorldPoint(event.target),
				PERM_DESTROY)) {
	...
	public void playerInteractEvent(final PlayerInteractEvent event) {
		...
		if ((shop.sellPrice >= 0) && ((shop.buyPrice < 0) || sameItem)) {
			...
			if (removedAmount < transactionStack.stackSize) {
				removedAmount += ModuleEconomy.tryRemoveItems(event.entityPlayer, transactionStack,
						transactionStack.stackSize - removedAmount);
			}
			wallet.add(shop.sellPrice);
			shop.setStock(shop.getStock() + 1);

			String price = APIRegistry.economy.toString(shop.sellPrice);
			ChatComponentTranslation msg = new ChatComponentTranslation("Sold %s x %s for %s (wallet: %s)", shop.amount,
			...
			ChatComponentTranslation msg = new ChatComponentTranslation("Bought %s x %s for %s (wallet: %s)",
