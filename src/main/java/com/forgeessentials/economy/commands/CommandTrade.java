...
public class CommandTrade extends ParserCommandBase {
	...
	public void parse(final CommandParserArgs arguments) throws net.minecraft.command.CommandException {
		...
		QuestionerCallback sellerHandler = response -> {
			if (response == null) {
			...
			QuestionerCallback buyerHandler = response1 -> {
				if (response1 == null) {
					arguments.error("Trade request timed out");
					return;
				...
				if (!ItemStack.areItemStacksEqual(currentItemStack, itemStack)
						|| !ItemStack.areItemStackTagsEqual(currentItemStack, itemStack)) {
					ChatOutputHandler.chatError(buyer.getPlayerMP(), Translator.translate("Error in transaction"));
					arguments
							.error(Translator.translate("You need to keep the item equipped until trade is finished!"));
					return;
				}
			...
			try {
				...
				if (itemStack.stackSize == 1) {
					message = Translator.format("Buy one %s for %s from %s?", itemStack.getDisplayName(),
							APIRegistry.economy.toString(price), arguments.sender.getName());
				} else {
					message = Translator.format("Buy %d x %s each for %s (total: %s) from %s?", itemStack.stackSize,
							itemStack.getDisplayName(), APIRegistry.economy.toString(price),
							APIRegistry.economy.toString(price * itemStack.stackSize), arguments.sender.getName());
				}
		...
		String message;
		if (itemStack.stackSize == 1) {
			message = Translator.format("Sell one %s for %s to %s?", itemStack.getDisplayName(),
					APIRegistry.economy.toString(price), buyer.getUsernameOrUuid());
		} else {
			message = Translator.format("Sell %d x %s each for %s (total: %s) to %s?", itemStack.stackSize,
					itemStack.getDisplayName(), APIRegistry.economy.toString(price),
					APIRegistry.economy.toString(price * itemStack.stackSize), buyer.getUsernameOrUuid());
		}
