...
public class SelectionHandler extends ServerEventHandler {
	...
	public static void sendUpdate(EntityPlayerMP player) {
		try {
			if (PlayerInfo.get(player).getHasFEClient()) {
				try {
					NetworkUtils.netHandler.sendTo(new Packet1SelectionUpdate(selectionProvider.getSelection(player)),
							player);
	...
	public void onClientConnect(final ClientHandshakeEstablished e) {
		TaskRegistry.runLater(() -> sendUpdate(e.getPlayer()));
	}
	...
	public void playerInteractEvent(PlayerInteractEvent event) {
		...
		if (event.action.equals(PlayerInteractEvent.Action.LEFT_CLICK_BLOCK)) {
			...
			String message = Translator.format("Pos1 set to %d, %d, %d", event.pos.getX(), event.pos.getY(),
					event.pos.getZ());
		...
		// right Click
			...
			String message = Translator.format("Pos2 set to %d, %d, %d", event.pos.getX(), event.pos.getY(),
					event.pos.getZ());
			ChatOutputHandler.sendMessage(player, message, EnumChatFormatting.DARK_PURPLE);
			event.setCanceled(true);
		}
