...
public class FEChunkLoader implements LoadingCallback {
	...
	public boolean forceLoadWorld(World world) {
		if (map.containsKey(world.provider.getDimensionId())) {
			...
			Ticket ticket = ForgeChunkManager.requestTicket(ForgeEssentials.instance, world,
	...
	public void ticketsLoaded(List<Ticket> tickets, World world) {
		// We don't care about reloading the chunks at load right now. The
		// filler needs to be restarted manually.
		...
		if (map.containsKey(world.provider.getDimensionId())) {
			...
			if (ticket.getModData().getInteger("count") == 0) {
				LoggingHandler.felog
						.debug(world.provider.getDimensionId() + " was removed fron the force loaded list.");
				ForgeChunkManager.unforceChunk(ticket, new ChunkCoordIntPair(0, 0));
				ForgeChunkManager.releaseTicket(ticket);
				map.remove(world.provider.getDimensionId());
				return true;
			} else {
				LoggingHandler.felog.debug(world.provider.getDimensionId() + " is still force loaded. "
						+ ticket.getModData().getInteger("count") + " requests remain.");
				return false;
			}
		} else {
