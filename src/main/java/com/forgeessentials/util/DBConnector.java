...
public class DBConnector {
	 ...
	 */
	public DBConnector(String name, DBConnector fallback, EnumDBType dType, String dbDefault, String dbFileDefault,
			boolean useFallback) {
		this.name = name;
	...
	public void loadOrGenerate(Configuration config, String cat) {
		...
		for (EnumDBType dbType : EnumDBType.values()) {
			...
			if (dbType.isRemote) {
				...
				props.put("database",
						config.get(newcat, "database", dbFileDefault, "this may be a file path as well."));
			}
	...
	public void write(Configuration config, String cat) {
		config.get(cat, "chosenType", dType.toString(), " valid types: " + StringUtils.join(EnumDBType.values(), ", "))
				.set(type.toString());
		...
		for (EnumDBType dbType : EnumDBType.values()) {
			...
			if (dbType.isRemote) {
				...
				config.get(newcat, "database", dbFileDefault, "this may be a file path as well.")
						.set(props.get("database").getString());
			}
