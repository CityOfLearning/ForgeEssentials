...
public class ModuleScripting extends ServerEventHandler implements ScriptHandler {
	...
	public void createDefaultPatternCommands() {
		...
		if (!PatternCommand.getPatternCommands().containsKey("god")) {
			...
			cmd.getPatterns().put("on @p",
					Arrays.asList(new String[] { "permcheck fe.commands.god.others", //
							"permset user @0 deny fe.protection.damageby.*", "$*/heal @0",
							"echo God mode turned ON for @0" }));
			cmd.getPatterns().put("off @p",
					Arrays.asList(new String[] { //
							"permcheck fe.commands.god.others", "permset user @0 clear fe.protection.damageby.*",
							"echo God mode turned OFF for @0", }));
			cmd.getPatterns().put("on",
					Arrays.asList(new String[] { //
							"permcheck fe.commands.god", "permset user @player deny fe.protection.damageby.*",
							"$*/heal @player", "echo God mode ON", }));
	...
	public void reloadScripts() {
		for (Entry<String, Map<String, List<String>>> entry : scripts.entrySet()) {
			...
			// File[] files = path.listFiles(new FilenameFilter() { @Override
			// public boolean accept(File dir, String
			// name) { return
			// FilenameUtils.getExtension(name).equalsIgnoreCase("txt");}});
			...
			for (Entry<String, List<String>> script : cronScripts.entrySet()) {
				...
				try {
					...
					LoggingHandler.felog
							.error(String.format("Error in script \"%s\": %s", script.getKey(), e.getMessage()));
				}
			}
		}
	}
