...
public class Grave implements Loadable {
	...
	public Grave(EntityPlayer player, List<EntityItem> drops, int xp) {
		...
		hasFencePost = PermissionManager.checkPermission(player, ModuleAfterlife.PERM_DEATHCHEST_FENCE);
		lastTick = System.currentTimeMillis();
		protTime = ServerUtil.parseIntDefault(
		...
		point.setY(WorldUtil.placeInWorld(player.worldObj, point.getX(), point.getY(), point.getZ(),
				hasFencePost ? 2 : 1));
		if (hasFencePost) {
	...
	public void interact(EntityPlayerMP player) {
		...
		player.playerNetServerHandler.sendPacket(new S2DPacketOpenWindow(player.currentWindowId, "minecraft:chest",
				invGrave.getDisplayName(), invGrave.getSizeInventory()));
