	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		try {
			if (args.length < 1) {
				...
				if ((args.length != 1) && (args.length != 2)) {
					if (args.length < (b0 + 3)) {
						...
						CommandBase.CoordinateArg argPitch = parseCoordinate(entity.rotationYaw,
								args.length > i ? args[i++] : "~", false);
						CommandBase.CoordinateArg argYaw = parseCoordinate(entity.rotationPitch,
								args.length > i ? args[i] : "~", false);
						float pitch;
						...
						if (entity instanceof EntityPlayerMP) {
							...
							WarpPoint pos = new WarpPoint(entity.worldObj.provider.getDimensionId(),
									argX.func_179629_b(), argY.func_179629_b(), argZ.func_179629_b(), pitch, yaw);
							...
							entity.setLocationAndAngles(argX.func_179628_a(), argY.func_179628_a(),
									argZ.func_179628_a(), f2, pitch);
							entity.setRotationYawHead(f2);
						}

						notifyOperators(sender, this, "commands.tp.success.coordinates",
								new Object[] { entity.getName(), Double.valueOf(argX.func_179628_a()),
					...
					Entity targetEntity = getEntity(sender, args[args.length - 1],
							sender.getCommandSenderEntity().getClass());
					if (targetEntity instanceof EntityPlayerMP) {
						WarpPoint pos = new WarpPoint(targetEntity.worldObj.provider.getDimensionId(),
								targetEntity.posX, targetEntity.posY, targetEntity.posZ, targetEntity.rotationPitch,
			...
			LoggingHandler.felog
					.warn("Encountered Error in FE Command TP class, likely no entity associated with command sender");
			npe.printStackTrace();
		}
	}
