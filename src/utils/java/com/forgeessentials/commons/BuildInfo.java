...
public abstract class BuildInfo {
	...
	public static final String BASE_VERSION = "1.8.5"; // update manually
														// because gradle is a
														// derp

	static {
		// Check for latest version asap
		checkVersionThread = new Thread(() -> doCheckLatestVersion());
		checkVersionThread.start();

		checkBuildTypesThread = new Thread(() -> doCheckBuildTypes());
		checkBuildTypesThread.start();
	}

	public static void cancelVersionCheck() {
		// Set to null, which will disable joining of the thread and kill any
		// possible delay
